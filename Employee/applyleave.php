...
<body bgcolor="#F0FFFF">
	...
	<table>
			...
			<?php
				$sql = "Select users.id, users.firstName, users.lastName, employee_leave.start, employee_leave.end, employee_leave.reason, employee_leave.status From users, employee_leave Where users.id = $id and employee_leave.id = $id order by employee_leave.token";
				$result = mysqli_query($conn, $sql);
